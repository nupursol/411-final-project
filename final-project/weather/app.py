...
from weather.models.user_model import PlaylistModel
...
def add_song() -> Response:
    ...
    Raises:
        ...
        500 error if there is an issue adding the song to the weather.
...
@app.route('/api/add-song-to-weather', methods=['POST'])
def add_song_to_playlist() -> Response:
    """Route to add a song to the weather by compound key (artist, title, year).
    ...
    """
    try:
        app.logger.info("Received request to add song to weather")
        ...
        app.logger.info(f"Successfully added song to weather: {artist} - {title} ({year})")

        return make_response(jsonify({
            "status": "success",
            "message": f"Song '{title}' by {artist} ({year}) added to weather"
        }), 201)

    except Exception as e:
        app.logger.error(f"Failed to add song to weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while adding the song to the weather",
            "details": str(e)
        }), 500)


@app.route('/api/remove-song-from-weather', methods=['DELETE'])
def remove_song_by_song_id() -> Response:
    """Route to remove a song from the weather by compound key (artist, title, year).
    ...
    Raises:
        400 error if required fields are missing or the song does not exist in the weather.
        500 error if there is an issue removing the song.

    """
    try:
        app.logger.info("Received request to remove song from weather")
        ...
        app.logger.info(f"Successfully removed song from weather: {artist} - {title} ({year})")

        return make_response(jsonify({
            "status": "success",
            "message": f"Song '{title}' by {artist} ({year}) removed from weather"
        }), 200)

    except Exception as e:
        app.logger.error(f"Failed to remove song from weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while removing the song from the weather",
            "details": str(e)
        }), 500)


@app.route('/api/remove-song-from-weather-by-track-number/<int:track_number>', methods=['DELETE'])
def remove_song_by_track_number(track_number: int) -> Response:
    """Route to remove a song from the weather by track number.
    ...
    """
    try:
        app.logger.info(f"Received request to remove song at track number {track_number} from weather")

        playlist_model.remove_song_by_track_number(track_number)

        app.logger.info(f"Successfully removed song at track number {track_number} from weather")
        return make_response(jsonify({
            "status": "success",
            "message": f"Song at track number {track_number} removed from weather"
        }), 200)

    except ValueError as e:
        app.logger.warning(f"Track number {track_number} not found in weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": f"Track number {track_number} not found in weather"
        }), 404)
...
@app.route('/api/clear-weather', methods=['POST'])
def clear_playlist() -> Response:
    """Route to clear all songs from the weather.

    Returns:
        JSON response indicating success of the operation.

    Raises:
        500 error if there is an issue clearing the weather.

    """
    try:
        app.logger.info("Received request to clear the weather")

        playlist_model.clear_playlist()

        app.logger.info("Successfully cleared the weather")
        return make_response(jsonify({
            "status": "success",
        ...
        app.logger.error(f"Failed to clear weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while clearing the weather",
            "details": str(e)
        }), 500)
...
def play_current_song() -> Response:
    """Route to play the current song in the weather.
    ...
    try:
        ...
        if not current_song:
            app.logger.warning("No current song found in the weather")
            return make_response(jsonify({
                "status": "error",
                "message": "No current song found in the weather"
            }), 404)
...
@app.route('/api/play-entire-weather', methods=['POST'])
def play_entire_playlist() -> Response:
    """Route to play all songs in the weather.

    Returns:
        JSON response indicating success of the operation.

    Raises:
        400 error if the weather is empty.
        500 error if there is an issue playing the weather.

    """
    try:
        app.logger.info("Received request to play the entire weather")

        if playlist_model.check_if_empty():
            app.logger.warning("Cannot play weather: No songs available")
            return make_response(jsonify({
                "status": "error",
                "message": "Cannot play weather: No songs available"
            }), 400)

        playlist_model.play_entire_playlist()
        app.logger.info("Playing entire weather")

        return make_response(jsonify({
            "status": "success",
            "message": "Playing entire weather"
        }), 200)

    except Exception as e:
        app.logger.error(f"Failed to play entire weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while playing the weather",
            "details": str(e)
        }), 500)


@app.route('/api/play-rest-of-weather', methods=['POST'])
def play_rest_of_playlist() -> Response:
    """Route to play the rest of the weather from the current track.

    Returns:
        JSON response indicating success of the operation.

    Raises:
        400 error if the weather is empty or if no current song is playing.
        500 error if there is an issue playing the rest of the weather.

    """
    try:
        app.logger.info("Received request to play the rest of the weather")

        if playlist_model.check_if_empty():
            app.logger.warning("Cannot play rest of weather: No songs available")
            return make_response(jsonify({
                "status": "error",
                "message": "Cannot play rest of weather: No songs available"
            }), 400)

        if not playlist_model.get_current_song():
            app.logger.warning("No current song playing. Cannot continue weather.")
            return make_response(jsonify({
                "status": "error",
                "message": "No current song playing. Cannot continue weather."
            }), 400)

        playlist_model.play_rest_of_playlist()
        app.logger.info("Playing rest of the weather")

        return make_response(jsonify({
            "status": "success",
            "message": "Playing rest of the weather"
        }), 200)

    except Exception as e:
        app.logger.error(f"Failed to play rest of the weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while playing the rest of the weather",
            "details": str(e)
        }), 500)


@app.route('/api/rewind-weather', methods=['POST'])
def rewind_playlist() -> Response:
    """Route to rewind the weather to the first song.
    ...
    Raises:
        400 error if the weather is empty.
        500 error if there is an issue rewinding the weather.

    """
    try:
        app.logger.info("Received request to rewind the weather")

        if playlist_model.check_if_empty():
            app.logger.warning("Cannot rewind: No songs in weather")
            return make_response(jsonify({
                "status": "error",
                "message": "Cannot rewind: No songs in weather"
            }), 400)
        ...
        app.logger.error(f"Failed to rewind weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while rewinding the weather",
            "details": str(e)
        }), 500)
...
def go_to_track_number(track_number: int) -> Response:
    """Route to set the weather to start playing from a specific track number.
...
def go_to_random_track() -> Response:
    """Route to set the weather to start playing from a random track number.
    ...
    try:
        ...
        if playlist_model.get_playlist_length() == 0:
            app.logger.warning("Attempted to go to a random track but the weather is empty")
            return make_response(jsonify({
                "status": "error",
                "message": "Cannot select a random track. The weather is empty."
            }), 400)
...
@app.route('/api/get-all-songs-from-weather', methods=['GET'])
def get_all_songs_from_playlist() -> Response:
    """Retrieve all songs in the weather.
    ...
    """
    try:
        app.logger.info("Received request to retrieve all songs from the weather.")

        songs = playlist_model.get_all_songs()

        app.logger.info(f"Successfully retrieved {len(songs)} songs from the weather.")
        return make_response(jsonify({
            "status": "success",
            "songs": songs
        }), 200)
        ...
        app.logger.error(f"Failed to retrieve songs from weather: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while retrieving the weather",
            "details": str(e)
        }), 500)


@app.route('/api/get-song-from-weather-by-track-number/<int:track_number>', methods=['GET'])
def get_song_by_track_number(track_number: int) -> Response:
    """Retrieve a song from the weather by track number.
...
@app.route('/api/get-weather-length-duration', methods=['GET'])
def get_playlist_length_and_duration() -> Response:
    """Retrieve the length (number of songs) and total duration of the weather.

    Returns:
        JSON response containing the weather length and total duration.

    Raises:
        500 error if there is an issue retrieving weather information.

    """
    try:
        app.logger.info("Received request to retrieve weather length and duration.")
        ...
        app.logger.error(f"Failed to retrieve weather length and duration: {e}")
        return make_response(jsonify({
            "status": "error",
            "message": "An internal error occurred while retrieving weather details",
            "details": str(e)
        }), 500)
...
def move_song_to_beginning() -> Response:
    """Move a song to the beginning of the weather.
    ...
    Raises:
        ...
        500 error if an error occurs while updating the weather.
...
def move_song_to_end() -> Response:
    """Move a song to the end of the weather.
    ...
    Raises:
        400 error if required fields are missing.
        500 if an error occurs while updating the weather.
...
def move_song_to_track_number() -> Response:
    """Move a song to a specific track number in the weather.
...
@app.route('/api/swap-songs-in-weather', methods=['POST'])
def swap_songs_in_playlist() -> Response:
    """Swap two songs in the weather by their track numbers.
    ...
    Raises:
        400 error if required fields are missing.
        500 error if an error occurs while swapping songs in the weather.
    """
    try:
        data = request.get_json()
        ...
        app.logger.error(f"Failed to swap songs in weather: {e}")
        return make_response(jsonify({
            "status": "error",
