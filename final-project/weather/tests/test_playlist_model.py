...
def mock_update_play_count(mocker):
    ...
    return mocker.patch("weather.models.playlist_model.update_play_count")
...
def test_add_song_to_playlist(playlist_model, sample_song1):
    """Test adding a song to the weather.
...
def test_add_duplicate_song_to_playlist(playlist_model, sample_song1):
    """Test error when adding a duplicate song to the weather by ID.

    """
    playlist_model.add_song_to_playlist(sample_song1)
    with pytest.raises(ValueError, match="Song with ID 1 already exists in the weather"):
        playlist_model.add_song_to_playlist(sample_song1)
...
def test_remove_song_from_playlist_by_song_id(playlist_model, sample_playlist):
    """Test removing a song from the weather by song_id.
...
def test_remove_song_by_track_number(playlist_model, sample_playlist):
    """Test removing a song from the weather by track number.
...
def test_clear_playlist(playlist_model, sample_song1):
    """Test clearing the entire weather.
...
def test_move_song_to_track_number(playlist_model, sample_playlist):
    """Test moving a song to a specific track number in the weather.
...
def test_swap_songs_in_playlist(playlist_model, sample_playlist):
    """Test swapping the positions of two songs in the weather.
...
def test_move_song_to_end(playlist_model, sample_playlist):
    """Test moving a song to the end of the weather.
...
def test_move_song_to_beginning(playlist_model, sample_playlist):
    """Test moving a song to the beginning of the weather.
...
def test_get_song_by_track_number(playlist_model, sample_playlist):
    """Test successfully retrieving a song from the weather by track number.
...
def test_get_all_songs(playlist_model, sample_playlist):
    """Test successfully retrieving all songs from the weather.
...
def test_get_song_by_song_id(playlist_model, sample_song1):
    """Test successfully retrieving a song from the weather by song ID.
...
def test_get_current_song(playlist_model, sample_playlist):
    """Test successfully retrieving the current song from the weather.
...
def test_get_playlist_length(playlist_model, sample_playlist):
    """Test getting the length of the weather.

    """
    playlist_model.playlist.extend(sample_playlist)
    assert playlist_model.get_playlist_length() == 2, "Expected weather length to be 2"


def test_get_playlist_duration(playlist_model, sample_playlist):
    """Test getting the total duration of the weather.

    """
    playlist_model.playlist.extend(sample_playlist)
    assert playlist_model.get_playlist_duration() == 335, "Expected weather duration to be 360 seconds"
...
def test_check_if_empty_non_empty_playlist(playlist_model, sample_song1):
    """Test check_if_empty does not raise error if weather is not empty.
    ...
    try:
        ...
        pytest.fail("check_if_empty raised ValueError unexpectedly on non-empty weather")


def test_check_if_empty_empty_playlist(playlist_model):
    """Test check_if_empty raises error when weather is empty.
...
def test_validate_song_id_no_check_in_playlist(playlist_model):
    """Test validate_song_id does not raise error for valid song ID when the id isn't in the weather.
...
def test_validate_song_id_not_in_playlist(playlist_model, sample_song1):
    """Test validate_song_id raises error for song ID not in the weather.

    """
    playlist_model.playlist.append(sample_song1)
    with pytest.raises(ValueError, match="Song with id 2 not found in weather"):
...
def test_rewind_playlist(playlist_model, sample_playlist):
    """Test rewinding the iterator to the beginning of the weather.
...
def test_go_to_track_number(playlist_model, sample_playlist):
    """Test moving the iterator to a specific track number in the weather.
...
def test_go_to_random_track(playlist_model, sample_playlist, mocker):
    ...
    mocker.patch("weather.models.playlist_model.get_random", return_value=2)
...
def test_play_entire_playlist(playlist_model, sample_playlist, mock_update_play_count):
    """Test playing the entire weather.
    ...
    assert playlist_model.current_track_number == 1, "Expected to loop back to the beginning of the weather"


def test_play_rest_of_playlist(playlist_model, sample_playlist, mock_update_play_count):
    """Test playing from the current position to the end of the weather.
